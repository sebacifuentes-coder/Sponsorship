...
- **Fase 4 (Implementación) — en curso, en Claude Code / VS Code.** ✅ **Épica 1 (Cimientos + Primer valor / TTFV) completa** (historias 1.1–1.5): stack Next.js 16 + Supabase + Vercel AI SDK + Tailwind/shadcn, estructura modular del doble plano, RBAC básico, ingesta de datos públicos (semilla tras `PublicDataPort`), motor de oportunidades + priorización versionada, y onboarding de primer valor con TTFV instrumentado. ✅ **Épica 2 (Contexto y personalización) completa** (historias 2.1–2.5, pusheadas a `main`): módulo `core/context` con derechos contratados (FR-15, limitan oportunidades), ADN de marca con pre-relleno IA tras `BrandDnaPort` (FR-16), objetivos de comunicación que afinan la priorización con boost versionado (FR-17), contexto adicional opcional (FR-18) y personalización integrada (oportunidades/concepto referencian derechos/ADN/objetivos; fallback "básica" sin contexto). Próximo: Épica 3. Detalle y prompt de arranque en `docs/HANDOFF-fase4-claude-code.md` (§1.5/§1.6 = lo ya construido).
